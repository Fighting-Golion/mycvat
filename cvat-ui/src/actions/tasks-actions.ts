...
function loadAnnotationsSuccess(task: any,bands: string): AnyAction {
    const action = {
...
export function loadAnnotationsAsync(
    ...
    return async (dispatch: ActionCreator<Dispatch>): Promise<void> => {
        try {
            ...
            console.log(bands);
            await task.annotations.upload(file, loader,bands);
        } catch (error) {
            dispatch(loadAnnotationsFailed(task, error));
            return;
        }

        dispatch(loadAnnotationsSuccess(task,bands));
    };
}
