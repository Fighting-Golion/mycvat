...
function AnnotationMenuComponent(props: Props & RouteComponentProps): JSX.Element {
    ...
    function onClickMenuWrapper(params: MenuInfo): void {
        ...
        }else if (params.key === Actions.AUTO_LABEL) {
            Modal.confirm({
                title: 'Do you want to start automatic labeling?',
                content: 'Current annotation will be lost. You are going to upload new annotations to this job. Continue?',
                okText: 'Continue',
                cancelText: 'Cancel',
                className: 'cvat-modal-content-auto-label',
                onOk: () => {
                    onClickMenu(params);
                },
            }); 
