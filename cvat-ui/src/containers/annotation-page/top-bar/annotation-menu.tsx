...
import getServer from 'cvat-server-wrapper'
...
function AnnotationMenuContainer(props: Props): JSX.Element {
    ...
    const onClickMenu = (params: MenuInfo): void => {
        ...
        if (action === Actions.EXPORT_TASK_DATASET) {
            ...
            history.push(`/tasks/${jobInstance.taskId}`); 
        } else if (action === Actions.AUTO_LABEL) {
            console.log(jobInstance);
            const serverProxy = getServer();
            console.log(serverProxy);
            serverProxy.server.note(jobInstance.taskId);
            alert('successfully created a button.');   
