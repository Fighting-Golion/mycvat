...
(() => {
    ...
    async function uploadAnnotations(session, file, loader,bands) {
        console.log(bands);
        const sessionType = session instanceof Task ? 'task' : 'job';
        if (!(loader instanceof Loader)) {
            throw new ArgumentError('A loader must be instance of Loader class');
        }
        await serverProxy.annotations.uploadAnnotations(sessionType, session.id, file, loader.name,bands);
    }
