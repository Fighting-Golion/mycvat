...
from cvat.apps.engine.g_dataset_docker2 import imageToDataset,BasicDataset
...
def net_test(cnn,test_loader):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            _,test_output = cnn(data)
            _,pred = torch.max(test_output,1)
            correct += torch.sum(pred == label.data)
    return correct / len(test_loader.dataset)

def train2(train_data):
    train_loader = _Data.DataLoader(dataset=train_data, batch_size=BATCH_SIZE, shuffle=True)
    print('this train process contain {0} samples'.format(len(train_data)))
    print('training...')
    cnn = Net()
    ...
    for epoch in range(EPOCH):
        count = 0
        sum_loss = 0
        for step, da in enumerate(train_loader):
            b_x = da['image']
            b_y = da['label']
            _,output = cnn(b_x)
        ...
        train_ls.append(train_score.item())
        loss_ls.append(sum_loss/count)

    return cnn,train_ls,loss_ls


def net_predict(cnn,predict_data):
    cnn.eval()
    predict_loader = _Data.DataLoader(dataset=predict_data, batch_size=BATCH_SIZE, shuffle=False)
    label_pred = np.zeros((10000),dtype=np.int32)
    with torch.no_grad():
        for i,data in enumerate(predict_loader):
            ...
            if len(pred) < BATCH_SIZE:
                label_pred[-len(pred ):] = pred
            else:
                label_pred[i * BATCH_SIZE:(i + 1) * BATCH_SIZE] = pred
    label_pred = label_pred.reshape(100, 100)
    return label_pred
...
class TaskViewSet(UploadMixin, viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['OPTIONS', 'POST', 'GET'], url_path=r'autoanno')
    def autoanno(self, request, pk):
        db_task = self.get_object() # call check_object_permissions as well
        task_data = db_task.data
        uploaded_files = task_data.get_uploaded_files()
        logger.info(uploaded_files)
    ...
    def annotations(self, request, pk):
        ...
        if request.method == 'GET':
            if JobViewSet.myMask_flag == 1:
                ...
                start_cla = 3100
                to_predict_data = imageToDataset(tiffiles_in,start_cla=start_cla)
                ...
                shapes['type'] = 'points'
                shapes['occluded'] = False
                shapes['z_order'] = 0
                shapes['rotation'] = 0.0
                shapes['points'] = ls_index
                shapes['id'] = 6
                shapes['frame'] = 0
                shapes['label_id'] = task_id
                shapes['group'] = 0
                shapes['source'] = 'manual'
                shapes['attributes'] = []
                if len(ls_index) > 0:
                    data['shapes'].append(shapes)
                JobViewSet.myMask_flag = 0
            else:
